...
// Helper único para fechar + salvar flag
function marcarEFechar(userId: string | undefined, onClose: (b: boolean) => void, temPeptideo: boolean) {
  try {
    localStorage.setItem('nv_bv_shown', '1');
...
export default function BoasVindasModal({ nome, userId, onClose }: Props) {
  ...
  // Se já foi mostrado antes, não renderiza e fecha imediatamente
  useEffect(() => {
  ...
  return (
    <div
      className="overlay"
      onClick={e => e.target === e.currentTarget && marcarEFechar(userId, onClose, false)}
    >
      <div style={{
        background: '#F7F7F7', borderRadius: 20, padding: '1.5rem',
        maxWidth: 520, width: 'calc(100% - 1.5rem)',
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              ...
              <h2 style={{
                ...
                letterSpacing: '-.04em', marginBottom: '.5rem'
              }}>
            ...
            <div style={{
              ...
              <div style={{
                fontSize: 14, fontWeight: 500, color: 'var(--tx)',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <button
                  onClick={() => marcarEFechar(userId, onClose, true)}
                  style={{
                    padding: '12px 16px', borderRadius: 12,
                    border: '2px solid var(--green)', background: 'var(--gp)',
                    cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 500,
                    color: 'var(--gm)', display: 'flex', alignItems: 'center', gap: 10
                  }}
                >
                  <span style={{ fontSize: '1.3rem' }}>✅</span>
                  <div style={{ textAlign: 'left' }}>
                    <div>Sim, já tenho os peptídeos</div>
                    <div style={{ fontSize: 11, opacity: .75, fontWeight: 400, marginTop: 1 }}>
                      Vou iniciar o protocolo agora
                    </div>
                  </div>
                </button>
                <button
                  onClick={() => setStep('nao_tem')}
                  style={{
                    padding: '12px 16px', borderRadius: 12,
                    border: 'none', background: '#F7F7F7',
                    cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 500,
                    color: 'var(--tx)', display: 'flex', alignItems: 'center', gap: 10
                  }}
                >
                  <span style={{ fontSize: '1.3rem' }}>⏳</span>
                  <div style={{ textAlign: 'left' }}>
                    <div>Ainda não tenho</div>
                    <div style={{ fontSize: 11, color: 'var(--ts)', fontWeight: 400, marginTop: 1 }}>
                      Quero saber onde encontrar
                    </div>
                  </div>
                </button>
              </div>
          ...
          <>
            <button
              ...
              style={{
                ...
                color: 'var(--ts)', fontSize: 13, fontFamily: 'inherit',
                marginBottom: '1rem'
              }}
            >
              ← Voltar
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <h2 style={{
                ...
                letterSpacing: '-.04em', marginBottom: '.375rem'
              }}>
                Onde encontrar peptídeos
              </h2>
              <p style={{ fontSize: 13, color: 'var(--tm)', lineHeight: 1.65 }}>
                A maioria precisa ser importada. Aqui estão as principais fontes usadas no Brasil.
              </p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: '1.5rem' }}>
              {FONTES.map(f => (
                <div key={f.titulo} style={{
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--tx)', marginBottom: 4 }}>
                      {f.titulo}
                    </div>
            ...
            <div style={{
              background: 'var(--gp)', border: '1px solid rgba(29,158,117,.2)',
              borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1.5rem',
              fontSize: 12, color: 'var(--gm)', lineHeight: 1.65
            }}>
              💡 Entre na comunidade do WhatsApp da Nuvita para receber indicações de fornecedores confiáveis.
            </div>
            <button
              onClick={() => marcarEFechar(userId, onClose, false)}
              className="btn btn-d fw"
