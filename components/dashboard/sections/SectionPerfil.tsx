...
export default function SectionPerfil({ answers, plan, onNavigate, userId, onPlanChange }: any) {
  ...
  return (
    <div style={{ maxWidth:640 }}>
      ...
      {/* Avatar + nome */}
      <div style={{ display:'flex', gap:20, alignItems:'center', background:'#FFFFFF', borderRadius:16, boxShadow:'0 1px 3px rgba(0,0,0,.06),0 2px 8px rgba(0,0,0,.04)', padding:'1.5rem', marginBottom:'1.25rem', border:'none' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:'1.25rem' }}>
        {[['📊','Check-ins totais',stats.checkins],['📅','Dias com registro',stats.dias],['🔥','Adesão esta semana',stats.semana]].map(([ico,lbl,val])=>(
          <div key={lbl} style={{ background:'#FFFFFF', borderRadius:12, boxShadow:'0 1px 3px rgba(0,0,0,.06),0 2px 8px rgba(0,0,0,.04)', padding:'1rem', border:'none', textAlign:'center' }}>
      ...
      <div style={{ background:'#FFFFFF', borderRadius:16, boxShadow:'0 1px 3px rgba(0,0,0,.06),0 2px 8px rgba(0,0,0,.04)', padding:'1.25rem', border:'none', marginBottom:'1.25rem' }}>
        ...
        <button style={{ marginTop:'1rem', fontSize:12, padding:'6px 14px', borderRadius:8, border:'none', background:'#F7F7F7', cursor:'pointer', fontFamily:'inherit', color:'var(--tm)' }}
