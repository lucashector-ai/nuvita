    ...
    <div style={{ maxWidth: 960, margin: '0 auto', paddingTop: '2.5rem', paddingBottom: '4rem' }}>
      ...
      {emailOpen && (
        <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center' }}
          ...
          <div style={{ background:'var(--bg)',borderRadius:16,padding:'2rem',width:380,boxShadow:'0 20px 60px rgba(0,0,0,.3)' }}>
            <div style={{ fontSize:15,fontWeight:600,marginBottom:'1.25rem' }}>Criar conta — {selectedPlan}</div>
            {/* Google */}
            <button onClick={handleGoogle}
              style={{ width:'100%',padding:'11px',borderRadius:10,border:'1.5px solid var(--border)',background:'var(--bg)',cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:500,display:'flex',alignItems:'center',justifyContent:'center',gap:10,marginBottom:12 }}>
