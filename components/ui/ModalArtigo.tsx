...
export default function ModalArtigo({ artigo, onClose }) {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20, backdropFilter:'blur(4px)' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background:'white', borderRadius:20, maxWidth:620, width:'100%', maxHeight:'85vh', overflow:'auto', boxShadow:'0 24px 64px rgba(0,0,0,.3)' }}
      >
        <div style={{ padding:'1.5rem 1.5rem 0', position:'sticky', top:0, background:'white', zIndex:1 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12, marginBottom:'1rem' }}>
            ...
            <button
              onClick={onClose}
              style={{ width:32, height:32, borderRadius:8, border:'none', background:'#F3F4F6', cursor:'pointer', fontSize:18, color:'#6B7280', flexShrink:0 }}
            >×</button>
          </div>
        ...
        <div style={{ padding:'1.25rem 1.5rem 1.5rem' }}>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            ...
            {artigo.pmid && (
              <a href={`https://pubmed.ncbi.nlm.nih.gov/${artigo.pmid}/`} target="_blank" rel="noopener noreferrer"
